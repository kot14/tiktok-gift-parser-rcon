...
export function processGift(data) {
  ...
  const COMBO_TIMEOUT = 2000; // 10 секунд - якщо минуло більше, вважаємо що комбо закінчилося
  ...
  if (data.giftType === 1) {
    ...
    const state = comboState.get(comboKey) || {
      lastRepeatCount: 0,
      lastProcessedTime: 0,
    };
    ...
    if (timeSinceLastProcess > COMBO_TIMEOUT && lastRepeatCount > 0) {
      console.log(
        `[GiftProcessor] Очищено старий стан для ${comboKey} (минуло ${timeSinceLastProcess}ms, більше ${COMBO_TIMEOUT}ms)`
      );
    ...
    // Якщо repeatCount зменшився або став 1 після більшого значення - це нова сесія
      ...
      currentRepeatCount < lastRepeatCount ||
      (currentRepeatCount === 1 &&
        lastRepeatCount > 1 &&
        timeSinceLastProcess > 2000)
    ) {
      console.log(
        `[GiftProcessor] Виявлено нову сесію подарунків для ${comboKey} (lastRepeat=${lastRepeatCount}, currentRepeat=${currentRepeatCount}, минуло ${timeSinceLastProcess}ms)`
      );
      comboState.delete(comboKey);
    ...
    // Якщо repeatCount той самий
      ...
      if (timeSinceLastProcess < 2000) {
        giftsToProcess = 1;
        console.log(
          `[GiftProcessor] Виявлено нову подію з тим самим repeatCount (${currentRepeatCount}) через ${timeSinceLastProcess}ms, обробляємо як 1 подарунок`
        );
      } else {
        ...
        if (currentRepeatCount === 1) {
          console.log(
            `[GiftProcessor] Виявлено нову сесію (repeatCount=1 після ${timeSinceLastProcess}ms)`
          );
          comboState.delete(comboKey);
          giftsToProcess = 1;
        } else {
          ...
          console.log(
            `[GiftProcessor] Пропущено: дублікат події (lastRepeat=${lastRepeatCount}, currentRepeat=${currentRepeatCount}, минуло ${timeSinceLastProcess}ms)`
          );
          return null;
        }
      }
    ...
    console.log(
      `[GiftProcessor] ${comboKey}: lastRepeat=${lastRepeatCount}, currentRepeat=${currentRepeatCount}, toProcess=${giftsToProcess}, timeSinceLast=${timeSinceLastProcess}ms`
    );

    // Оновлюємо стан тільки після успішного обчислення
    comboState.set(comboKey, {
    ...
    giftsToProcess = currentRepeatCount;
    console.log(
      `[GiftProcessor] Одноразовий подарунок ${comboKey}: toProcess=${giftsToProcess}`
    );
  }
